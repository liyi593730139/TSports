...
func runLocal(cmd string, conn net.Conn) {
	...
	encoding := tmps[0]
	if strings.HasPrefix(strings.ToLower(tmps[1]), "uname") {
		conn.Write([]byte(runtime.GOOS))
		return
	}
	if strings.HasPrefix(strings.ToLower(tmps[1]), "pwd") { //+
		strings.ToLower(tmps[1])
		...
		conn.Write([]byte(abspath))
	}
	cmd = "/c " + tmps[1]
	cmds := strings.Split(cmd, " ")
	for i := 0; i < len(cmds); i++ {
		cmds[i] = strings.Replace(cmds[i], "%20", " ", -1)
	}
	var out bytes.Buffer
	exeCmd := exec.Command("cmd", cmds...)
	exeCmd.Stdout = &out
	exeCmd.Stderr = &out
	exeCmd.Start()
	exeCmd.Wait()
	if strings.HasPrefix(runtime.GOOS, "win") && encoding != "GBK" {
		dec := mahonia.NewDecoder("gbk")
		//from GBK to UTF-8
		ret, ok := dec.ConvertStringOK(out.String())
		if !ok {
...
func getDir(path string, conn net.Conn) {
	...
	*cancel = false
	...
	if len(dirs) == 0 {
		...
		err := checkDir(remoteDir, localDir, conn, cancel)
		if err != nil {
			print("%s\n", err.Error())
		}
		conn.Write([]byte("end"))
 ...
 * @param localDir: 远端路径
 */
func checkDir(remoteDir, localDir string, conn net.Conn, cancel *bool) error {
	defer func() {
		if r := recover(); r != nil {
			print("Runtime error caught : %v \n", r)
		}
	...
	for _, dir := range dirs {
		...
		if dir.IsDir() {
			...
			if len(tmpdirs) == 0 {
				...
				str := string(buf[:n])
				if strings.HasPrefix(str, "cancel") {
					*cancel = true
				}
			} else {
				checkDir(remoteStr, localStr, conn, cancel)
			}
		} else {
...
func wsync(path string, conn net.Conn) {
	...
	*cancel = false
	...
	if len(dirs) == 0 {
		...
		err := wsyncCheckDir(logMap, newLogMap, remoteDir, localDir, conn, cancel)
		if err != nil {
			print("%s\n", err.Error())
		}
	}
	...
	buf := make([]byte, bufferSize)
	conn.Write([]byte("Log " + logFileName + "@-%-@" + logDir + "/" + curTime + ".log")) //+
	n, err := conn.Read(buf)
	if err != nil {
		return
	}
	...
	conn.Write([]byte("Log " + statusName + "@-%-@" + logDir + "/" + "meta.log")) //+
	n, err = conn.Read(buf)
	if err != nil {
		return
	}
 ...
 */
func wsyncCheckDir(logMap map[string]fileinfo, newLogMap [3]map[string]fileinfo, remoteDir, localDir string, conn net.Conn, cancel *bool) error {
	defer func() {
		if r := recover(); r != nil {
			print("Runtime error caught : %v \n", r)
		}}()
	...
	for _, dir := range dirs {
		...
		if dir.IsDir() {
			...
			if len(tmpdirs) == 0 {
				...
				wsyncCheckDir(logMap, newLogMap, remoteStr, localStr, conn, cancel)
			}
		} else {
			remoteStr := remoteDir + "\\" + dir.Name()
			localStr := localDir + "/" + dir.Name()
